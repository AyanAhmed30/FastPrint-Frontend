...
const apiService = {
  ...
  async saveSettings(data) {
    ...
    if (!response.ok) throw new Error(resData.message || "Failed to save profile");
    return resData;
  },
...
const LoadingSpinner = () => (
  <div className="flex justify-center items-center">
    <div className="relative">
      ...
      <div className="absolute top-0 left-0 w-8 h-8 border-4 border-transparent border-r-blue-400 rounded-full animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }}></div>
    </div>
  </div>
);

// Floating Label Input Component
const FloatingLabelInput = ({ name, label, type = "text", value, onChange, disabled, placeholder }) => {
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen py-4 sm:py-6 md:py-8 lg:py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#eef4ff] via-[#f8faff] to-[#fef6fb] relative overflow-hidden">
        ...
        <div className={`max-w-4xl mx-auto p-4 sm:p-6 md:p-8 lg:p-10 rounded-lg sm:rounded-xl lg:rounded-2xl shadow-lg sm:shadow-xl flex flex-col gap-6 sm:gap-8 lg:gap-10 bg-gradient-to-r from-[#ffe4ec] via-[#fdfdfd] to-[#e0f3ff] backdrop-blur-sm border border-white/50 relative z-10 transition-all duration-700 transform ${
          isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
        }`}>

          {/* Enhanced Message Alert */}
          {message && (
            <div className={`p-4 rounded-xl text-center font-medium transition-all duration-500 transform animate-slide-down text-sm sm:text-base shadow-lg border-2 ${
          ...
          {/* Profile Details Section with Stagger Animation */}
              ...
              {[
                ...
                <div key={name} 
                  ...
                  <FloatingLabelInput
                    ...
                    disabled={loading}
                    placeholder={name === 'password' ? 'Enter new password' : `Enter your ${label.toLowerCase()}`}
                  />
                </div>
              ))}
            </div>
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              {[
                { value: "personal", label: "Personal", description: "I print books for personal use or as gifts.", icon: PersonalIcon },
                { value: "business", label: "Business", description: "I publish books for business or resale.", icon: BusinessIcon },
                ...
                <label key={value} 
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                    ...
                    <div className="relative overflow-hidden rounded-xl">
                      <img src={icon} alt={label} className="h-10 w-10 sm:h-12 sm:w-12 object-contain transition-all duration-300 group-hover:scale-110" />
                  ...
                  <div className="flex-1 ml-6 sm:ml-0">
                    <h3 className="text-black font-semibold text-sm sm:text-base mb-1 transition-colors duration-300 group-hover:text-blue-700">{label}</h3>
                    <p className="text-black/70 text-xs sm:text-sm transition-colors duration-300 group-hover:text-black">{description}</p>
                  </div>
                </label>
              ))}
            </div>
